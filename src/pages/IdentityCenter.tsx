...
const InvitationCard: React.FC<UserComponentProps> = ({ gameheadsID }) => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        console.error(`Invitation Card Error: ${error?.message}`);
      }
    };
  ...
  const profilePicture =
    userData.profile.profile_picture || "/graphics/default-profile.png";
  ...
  return (
    ...
    >
      ...
      />
      <span
        className="text-sm sm:text-md text-links"
      ...
      {profile.users_endorsed_by.length > 0 ? (
        <div>
          ...
          <div className="bg-cream p-8">
            <h1 className="text-2xl sm:text-3xl font-bold my-4 text-center sm:text-left text-headings">
              Identity Center
            </h1>
            <p className="text-body mb-8 text-sm sm:text-base">
              This is your home base for keeping your identity verified with
              Gameheads.
            </p>
            <Button
            ...
            <div className="flex flex-col gap-3 mt-8">
              ...
              {profile.users_endorsed_by.length > 0 ? (
                profile.users_endorsed_by.map((gameheadsID: string) => (
                  <InvitationCard key={gameheadsID} gameheadsID={gameheadsID} />
                ))
              ) : (
                <div>
                  Looks like you haven{"'"}t been invited and/or endorsed by anyone yet! Time
              ...
              {profile.users_endorsed.length > 0 ? (
                profile.users_endorsed.map((gameheadsID: string) => (
                  <InvitationCard key={gameheadsID} gameheadsID={gameheadsID} />
                ))
              ) : (
                <div>You{"'"}ve yet to make your first invite. Make sure you only invite Gameheads students/alums!</div>
              )}
            </div>
          </div>
